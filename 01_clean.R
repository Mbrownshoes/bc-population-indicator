...
popn_bc <- "Population_Estimates.csv"
...
rd <- combine_nr_rd() %>% ## get districts layer for bcamps
...
popn_change <- popn %>% 
  ...
  group_by(Regional_District) %>% 
  mutate(popchange = Total-lag(Total)) %>% 
  mutate(percchange = round((popchange/lag(Total) * 100), digits = 2)) %>% 
  filter(Year == 2016) %>% 
  select(-Year) 
...
save(catlab, bctot, popn_gv, popn_rest, rd, popsummary, file = "tmp/sumdata.RData")
